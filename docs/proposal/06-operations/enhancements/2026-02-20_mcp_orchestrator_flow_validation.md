...
## Issue 12: Auto-unread trigger fan-out can create trigger storms and workstation-level lag
### Evidence
- After callback success was restored, `trigger_jobs` volume rapidly increased with many `trg_auto_unread_*` rows across active test threads.
- Snapshot during live run showed large concurrent backlog:
  - `callback_delivered=105`, `fallback_spawn=18`, `triggering=14` (plus additional pending statuses).
- Top message-heavy threads grew quickly (`30+` messages per thread) without human interaction.
- Spawned fallback flows fan out additional `codex exec` processes, which can start MCP sidecars repeatedly and cause noticeable CPU/RAM pressure.

### Impact
- Host/IDE can become sluggish or crash from process churn and high event throughput.
- Orchestrator sessions can appear to "cut off" due to environment instability under load.

### Suspected root cause
- Unread reconciliation enqueues trigger jobs for every dormant participant seeing unread deltas, even when the new unread messages are themselves generated by prior automation waves.
- In multi-participant threads, one message can repeatedly wake multiple participants, creating N-way cascades.

### Immediate mitigation (operator)
1. Stop `supervisor-worker` to halt new trigger scheduling.
2. Use `operator-cli` to inspect affected threads and close/archive noisy test threads.
3. Avoid broad participant sets for stress tests; use 1:1 orchestrator->worker threads for validation.

### Engineering follow-up
- Add auto-unread rate/budget guard at scheduler level (per thread + per participant + time window).
- Add suppression/TTL metadata for automation-originated unread events beyond callback events.
- Add circuit-breaker mode when trigger backlog exceeds threshold (pause auto-unread scheduling, require operator resume).
- Add explicit test coverage for fan-out scenarios (3+ participants posting completion updates).
