...
| F-08 | Escalation markers and owner-assigned escalation flow are supported | PRD FR-9, Backlog 5.2 | Scenario test showing escalation + unblock path | [ ] | [x] | Escalate/unblock exists (`apps/operator-cli/src/service.ts`, `apps/operator-cli/src/main.test.ts`), but owner-assigned escalation semantics are not yet implemented |
...
| D-04 | Backward-compatible additive changes do not break existing payload consumers | PRD FR-15, Protocol §9, Backlog 4.5 | Compatibility suite with baseline fixtures | [ ] | [x] | No dedicated backward-compatibility fixture suite found; add compatibility tests for additive changes |
...
| R-07 | Force override requires explicit operator intent and is fully audited | PRD FR-18, Runtime specs, Backlog 6.4 | Override scenario tests + audit entries | [ ] | [x] | Force override is implemented (`apps/supervisor-worker/src/runtime-trigger-executor.ts`) but dedicated audit-entry assertions for override paths are missing |
...
| ID | Acceptance Criterion | Source | Evidence Required | Pass | Fail | Notes |
|---|---|---|---|---|---|---|
| O-01 | No message loss after server acknowledgement under transient failure scenarios | PRD NFR-1, Backlog 4.3 | Fault-injection + replay tests | [ ] | [x] | Concurrency/idempotency tests exist, but explicit fault-injection replay coverage is missing |
...
| O-03 | Correlation IDs are present across API/worker logs and trace paths | PRD NFR-5, Backlog 4.1 | Structured log assertions | [ ] | [x] | API request IDs are logged (`apps/bridge-api/src/app.ts`), but end-to-end API->worker correlation assertions are missing |
