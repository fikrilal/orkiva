...
|---|---|---|---|
| F-08 | Escalation flow lacks owner-assigned semantics | Add explicit escalation owner fields/commands (assign, reassign, read current owner), enforce transition and authority rules, and add scenario tests for escalate -> assign -> unblock/close flow. | `apps/operator-cli/src/commands.ts`, `apps/operator-cli/src/service.ts`, `packages/protocol/src/v1/thread.ts`, `apps/operator-cli/src/main.test.ts` |
